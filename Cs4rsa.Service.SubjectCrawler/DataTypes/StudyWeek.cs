...
namespace Cs4rsa.Service.SubjectCrawler.DataTypes
{
    public class StudyWeek
    {
        ...
        public StudyWeek(string studyWeek)
        {
            ...
            var startAndEnd = studyWeek.Split(separatingStrings, StringSplitOptions.RemoveEmptyEntries);
            StartWeek = int.TryParse(startAndEnd[0], out var result) ? result : 0;
        ...
        {
            var lwAmount = GetLearnWeekAmount(start, end);
            return lwAmount >= 5 && lwAmount < 10 && start == 1;
        }
